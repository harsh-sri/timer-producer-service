...
export class TimerController extends BaseController {
  ...
  async addTimer(
    @Body(new ValidationPipe()) createTimerDto: CreateTimerDto,
  ): Promise<BaseResponse> {
    const { timerId, timerStatus, timeLeft } =
      await this.timerService.publish(createTimerDto);
  ...
  async getTimer(@Param() timerId: GetTimerDto): Promise<BaseResponse> {
    try {
      ...
      const validatedTimerId: GetTimerDto = await validationPipe.transform(
        timerId,
        {
          type: 'custom',
      ...
      const { timerStatus, timeLeft } =
        await this.timerService.getTimer(validatedTimerId);
      return new BaseResponse({
